...
```{r}
model_cols <- c(demog_vars, "age_mean_ctr", "total_months_worked_ctr", "depression")
model_df <- 
data_imputed %>% 
  rename_with(~str_replace_all(., "^subs_", ""), .cols = starts_with("subs_")) %>% 
  pivot_longer(
...
model_list <- tribble(
  ~name, ~frml,
  "Base Model", depression ~ score,
  "Model w/ Age", depression ~ score + age_mean_ctr,
  "Model w/ Age & Sex", depression ~ score + age_mean_ctr + sex, 
   "Model 1", depression ~ score + age_mean_ctr + sex + civil_status,
  "Model 2", depression ~ score + age_mean_ctr + sex + total_months_worked_ctr,
   "Model 3", depression ~ score + age_mean_ctr + sex + infected_c19
) %>% 
  mutate(
