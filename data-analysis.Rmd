...
# # get PHQ-9
data %>%
  mutate(across(.cols = all_of(phq_cols),
                ~ case_when(. == "Not at all" ~ 0L,
                            . == "Several days" ~ 1L, 
                            . == "More than half the days" ~ 2L, 
                            . == "Nearly everyday" ~ 3L) ,
                .names = "{.col}_i"))  %>% 
  select(matches("^phq\\d+_i"))
# smpl <- 
#   data %>%
#     mutate(across(.cols = all_of(phq_cols), factor))
# data[,phq_score := rowSums(data[,phq_cols, with = FALSE])]
```
...
tbl1 <- CreateTableOne(vars = demog_vars, data = data)
